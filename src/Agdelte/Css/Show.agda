...
{-# COMPILE JS showFloat = f => { if (Number.isNaN(f)) { console.warn("showFloat: NaN replaced with 0"); return "0"; } if (!isFinite(f)) { console.warn("showFloat: Infinity replaced with 0"); return "0"; } const s = String(f); return s.includes('e') ? f.toFixed(6).replace(/\.?0+$/, '') : s; } #-}
