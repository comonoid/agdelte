...
-- | tipId: unique ID for aria-describedby linkage
-- | trigger: content that triggers tooltip
-- | tip: tooltip text
-- | position: where to show tooltip
tooltip : ∀ {M A}
        → String             -- tooltip ID (for aria-describedby)
...
-- | Help icon (?) with tooltip.
helpIcon : ∀ {M A} → String → String → Node M A
helpIcon tipId tip =
  tooltip tipId
...
infoIcon : ∀ {M A} → String → String → Node M A
infoIcon tipId tip =
  tooltip tipId
