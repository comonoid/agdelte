...
> **⚠️ ЧАСТИЧНО ЗАМЕНЕНО (2026-06).** Решение по хранилищу изменилось: ядро строится на
> **WAL + in-memory, типизированные записи (НЕ Postgres)** — см.
> [`adr/0001-storage-wal-in-memory.md`](adr/0001-storage-wal-in-memory.md),
> [`concepts/storage-model.md`](concepts/storage-model.md),
> [`plan-crm-core.md`](plan-crm-core.md). Поэтому **SQL-ориентированные части ниже
> (§5.4–§5.8 DDL, §13 в части БД, §14.3/§14.4) — это запасной Postgres-путь (scale-out),
> а не текущая ветка.** Доменная модель (сущности/связи §5, события §6, контракт API §13,
> охват вертикалей) остаётся в силе. Перед работой читать доки выше, не §14 как план.
