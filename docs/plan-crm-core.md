...
- [x] **WAL: единица записи = ТРАНЗАКЦИЯ + length-prefix фрейминг (#1, B).** Весь `List Op`
      транзакции = **одна запись `lp(concat lp(serializeOp opᵢ))` + один `fsync`** (`frameTxn`).
      Рекавери читает length-prefix записи: рваный **хвост** дропается **целиком** (атомарность);
      `\n`/`:`/`|` в payload безопасны. ✓ доказано тестом (2-op запись реплеится атомарно).
- [x] **⚠️ A (находка сверх плана): ленивый UTF-8-декод лога (`readWalLog`).** Строгий
      `TIO.readFile` + `readFileSafe` бросал на рваном UTF-8-хвосте → `""` → **потеря всей
      истории**. Теперь raw-bytes → `decodeUtf8With lenientDecode` → битый хвост = U+FFFD,
      его дропает фрейминг. ✓ тест с дописанным `0xFF` подтверждает: ранние записи **выживают**.
- [x] **⚠️ D (находка сверх плана): порча в СЕРЕДИНЕ лога → отказ старта.** Полностью
      присутствующая, но недекодируемая запись → `die` (refuse to start), **не** молчаливый
      пропуск (иначе расхождение состояния). ✓ тест ловит `die` через `tryCatch`.
...
- [x] **Fault-injection тест** `server/WalRecoveryTest.agda` (#8) — **6/6 PASS на GHC**
      (`npm run test:walrec`): commit, выживание после рваного+битого хвоста, реконструкция
      `nextId`, отказ при порче в середине. `live ≡ replay` — by construction (`emit`=тот же
      `apply`, что и реплей); отдельный property-тест не понадобился.
...
- **краш-тестирование recovery** — ✓ есть `server/WalRecoveryTest.agda` (рваный+битый хвост,
  порча в середине); расширять при изменении формата WAL. Что НЕ покрыто: одновременный краш
  процесса (только симуляция обрыва файла), частичный `fsync` директории.
