...
- [x] **ДЕКОМПОЗИЦИЯ НА БИБЛИОТЕКИ (2026-06-16).** Монолит разрезан на зарегистрированные
      Agda-либы в `~/.agda/`: `agdelte-store` (стор), `agdelte-payments` (ЮKassa-клиент),
      `agdelte-crm` (CRM, бывш. `services-core/`), `agdelte-courses` (легаси видео-платформа) +
      `agdelte` = чистый фреймворк. Имена модулей сохранены. App-слой (`server/`, cabal, gen, docs)
      остался в этом репо. Раскладка/стражи — в `CLAUDE.md`. Аудит рефактора: 10 находок, все LOW
      (гигиена конфигов/доков), декомпозиция признана состоятельной. Тесты: `store-txn` 7/7
      (generic Txn реюзабелен), `courses-smoke` 6/6 (старая база крутится на сторе).
