...
| Категория | Количество фич | Сложность | Общие трудозатраты |
|-----------|----------------|-----------|-------------------|
| Тривиально | ~16 | 0-1 | ~2 недели |
| Небольшая работа | ~5 | 2-3 | ~2 недели |
| Проектирование | ~5 | 4-6 | ~2 месяца |

**Итого на паритет с Vue 3 по фичам: ~3 месяца**

Но! Vue 3 разрабатывался годами командой. Разница:

- **Vue:** Нужно изобретать реактивность, Virtual DOM, компилятор
- **Agdelte:** Signal/Event — известная модель, Runtime проще
...
**Архитектура Agdelte не создаёт дополнительной сложности для реализации фич Vue 3; некоторые фичи проще (cleanup, time-travel), некоторые требуют других паттернов (локальное состояние), но все реализуемы с сопоставимыми трудозатратами.**
