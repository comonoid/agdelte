...
> **Расширение:** этот документ — опциональное расширение базовой архитектуры из `architecture.md`.
>
> **Предусловие:** знакомство с Signal, Event, App, и принципом "всё IO — это Event".
...
| Примитив | Подписка | Событие | Отписка |
|----------|----------|---------|---------|
| `interval n` | Запустить таймер | `⊤` каждые n мс | Остановить |
| `request r` | Отправить HTTP | `Response` | Отменить |
...
data Progress (A : Set) : Set where
  Running  : ℕ → Progress A      -- процент выполнения (0-100)
  Done     : A → Progress A       -- результат
  Cancelled : Progress A          -- отменено
  Failed   : String → Progress A  -- ошибка
...
```agda
-- Задержка на N мс, затем одно событие
delay : ℕ → Event ⊤
delay ms = interval ms  -- один тик через ms (см. примечание)

-- Примечание: для точной семантики delay нужен oneshot-вариант interval
-- delayOnce : ℕ → Event ⊤ — срабатывает ровно один раз

-- Монадные операции для Event
pure : A → Event A
pure a .now  = [ a ]
pure a .next = never

_>>=_ : Event A → (A → Event B) → Event B  -- flatMap/bind
_>>_  : Event A → Event B → Event B        -- sequence (игнорировать результат первого)
e1 >> e2 = e1 >>= const e2

-- Переключиться на новый Event при каждом событии
switchMap : (A → Event B) → Event A → Event B

-- Поток изменений Signal (событие при каждом изменении значения)
...
| STM паттерн | Аналог в Agdelte |
|-------------|------------------|
| `TVar` (следить за изменениями) | `Signal` |
| `retry` (ждать условие) | `if cond then worker ... else never` |
| `orElse` (альтернатива) | `race` |
| Атомарность | `update` атомарен |
...
retryWorker maxRetries fn a = go 0 100
  where
    ...
    go n delayMs =
      if n >= maxRetries
      then pure (Left "Max retries exceeded")
      else race
...
record App (Msg : Set) (Model : Set) : Set where
  field
    ...
    workers : Model → List WorkerSpec  -- опционально: явное управление пулом
```

### Автоматическое управление

Для большинства случаев достаточно `events`:
...
```
architecture.md                    arch-concurrency.md
───────────────                    ───────────────────
...
**Worker = ещё один Event-примитив.** Конкурентность не меняет архитектуру — она добавляет новые примитивы, следующие той же модели:
